...
| Authentication — service | Strong service authN (PCI DSS 8.6) | Internal OAuth/JWT or mTLS-bound service identity assertions; Workload Identity for GCP service auth; no long-lived static service account keys in production | Auth config baseline, token verification tests, Workload Identity config export |
...
| Customer responsibility acknowledgment | Written service provider responsibility statement required (PCI DSS 4.0 Req 12.9.1) | Shared Responsibility Agreement provided to each tenant at onboarding; reviewed annually | Executed agreements per tenant, agreement template, annual re-execution records |
...
- Annual Shared Responsibility Agreement review and re-execution per tenant.
